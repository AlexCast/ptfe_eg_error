...
// PTFE_brdf_spec.c
// gcc PTFE_brdf_spec.c -o brdf_spec.exe -lm
...
int main(int argc, char *argv[]) {
	...
	if (argc != 4) {
		printf ("Usage: %s inputfile thetas phis\n", argv[0]);
		exit (-1);
	}
...
//		printf("\n\n");
	...
	static double brdf[32851][66] = {0};
	for(lb = 0; lb < 66; lb++) {
		for(c = 0; c < 32851; c++) {
			brdf[c][lb] = ZernikeExpansion_BRDF_Model(
							thetai_v[c], 
							thetas, 
							phii_v[c], 
							phis,
