...
// PTFE_brdf.c
// gcc PTFE_brdf_geom.c -o brdf_geom.exe -lm
...
double main(int argc, char *argv[]) {
	...
	if (argc != 7) {
		printf ("Usage: %s inputfile thetai phii thetas phis lambda\n", argv[0]);
		exit (-1);
	}
 
	fpi = fopen(argv[1], "r");
	double thetai = atof(argv[2]);
	double phii = atof(argv[3]);
	double thetas = atof(argv[4]);
	double phis = atof(argv[5]);
...
//		printf("\n\n");
	...
	double brdf = 0;
	brdf = ZernikeExpansion_BRDF_Model(
						thetai, 
						thetas, 
						phii, 
						phis,
